      ...
      <section id="work" className="relative z-10 mx-auto max-w-7xl scroll-mt-20 px-6 py-28 lg:px-10 lg:py-40">
        ...
        <article className="group relative mb-8 overflow-hidden rounded-[2rem] border border-white/[0.1] bg-white/[0.025] transition duration-500 hover:-translate-y-1 hover:border-white/20">
          ...
          <div className="relative grid min-h-[620px] lg:grid-cols-[1.05fr_.95fr]">
            <div className="flex flex-col justify-between border-b border-white/[0.07] p-8 sm:p-12 lg:border-b-0 lg:border-r"><div className="flex items-center justify-between"><span className="text-[10px] font-semibold uppercase tracking-[0.24em] text-white/25">Current build / SYNAPSE</span><span className="rounded-full border border-white/10 px-3 py-1.5 text-[9px] text-white/30">01</span></div><div className="py-14"><div className="mb-6 flex items-center gap-3"><span className="flex h-12 w-12 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.05] text-lg font-semibold">S</span><span className="text-[10px] uppercase tracking-[0.22em] text-white/30">Local-first workload planner</span></div><h3 className="max-w-2xl text-5xl font-semibold tracking-[-0.065em] sm:text-7xl">Turn workload<br /><span className="text-white/30">into a plan you can follow.</span></h3><p className="mt-7 max-w-xl text-sm leading-7 text-white/40 sm:text-base">SYNAPSE turns deadlines, estimated effort, priority, and realistic available time into a deterministic seven-day plan — then recalculates as work changes.</p></div><div className="flex flex-wrap gap-2">{projects[0].tags.map((tag) => <span key={tag} className="rounded-full border border-white/[0.08] bg-black/20 px-3 py-1.5 text-[10px] text-white/30">{tag}</span>)}</div></div>
            <div className="relative flex flex-col justify-between p-8 sm:p-12"><div className="absolute right-10 top-14 h-72 w-72 rounded-full border border-white/[0.07] transition duration-700 group-hover:scale-110 group-hover:rotate-6" /><div className="absolute right-28 top-32 h-36 w-36 rounded-full border border-white/[0.09] transition duration-700 group-hover:scale-125" /><div className="relative"><p className="mb-8 text-[10px] uppercase tracking-[0.22em] text-white/25">Planning loop</p><div className="space-y-5">{[["01", "CAPTURE", "Deadline + effort + priority"], ["02", "UNDERSTAND", "Capacity and workload pressure"], ["03", "ACT", "Next task and seven-day plan"], ["04", "RECALCULATE", "Complete work and rebalance"]].map(([num, title, text]) => <div key={num} className="flex gap-5 border-b border-white/[0.06] pb-5"><span className="font-mono text-[9px] text-white/20">{num}</span><div><p className="text-sm text-white/60">{title}</p><p className="mt-1 text-[10px] text-white/25">{text}</p></div></div>)}</div></div><div className="relative flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/[0.08] bg-black/20 p-4"><span className="text-[10px] text-white/30">Local-first · deterministic · explainable</span><a href={projects[0].href} target="_blank" rel="noreferrer" className="rounded-full bg-white px-4 py-2 text-[10px] font-semibold text-black transition hover:bg-white/90">Open planner <Arrow /></a></div></div>
          </div>
        </article>
